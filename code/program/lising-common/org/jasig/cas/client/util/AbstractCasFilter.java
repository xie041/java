...
public abstract class AbstractCasFilter extends AbstractConfigurationFilter {
    ...
    private String serverName = "http://www.tourie.com";
    ...
    public final void init(final FilterConfig filterConfig) throws ServletException {
        if (!isIgnoreInitConfiguration()) {
        	//修改serverName从配置文件properties中获取，不再从xml中获取，太麻烦了
            //约定客户端必须有一个配置文件，lising-frame.properties
            //#当前项目IP地址
			//current_project_ip = http://localhost:8080/
			//#子项目IP地址及工程名称
			//current_project_name = lising-frame/
        	//原先的写法
//        	setServerName(getPropertyFromInitParams(filterConfig, "serverName", null));
//        	setServerName(ProjectUtil.getSsoLoginSuccessReturnBackAddress());
    ...
    protected final String constructServiceUrl(final HttpServletRequest request, final HttpServletResponse response) {
    	...
    	if(request == null){
    		...
    		serverName = ProjectUtil.getCurrentProjectUrl(request);
    		serverName = serverName.substring(0, serverName.length()-1);
    	}
        return CommonUtils.constructServiceUrl(request, response, this.service,serverName, this.artifactParameterName, this.encodeServiceUrl);
    }
